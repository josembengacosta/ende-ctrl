...
function determinarCategoriaFinal(){
  ...
  // --- Testar Social 1 ---
  const r1 = REGRAS_SOCIAL['bt-social1'];
  const s1DentroSmax = smax >= r1.smaxMin && smax <= r1.smaxMax;
  const s1DentroConsumo = consumoMensal <= r1.consumoMax;
  result.s1.dentro = s1DentroSmax && s1DentroConsumo;
  result.s1.falhaSmax = !s1DentroSmax;
  result.s1.falhaConsumo = !s1DentroConsumo;
  result.s1.falhaAmbos = !s1DentroSmax && !s1DentroConsumo;
  result.s1.sugestao = r1.sugestaoSmaxAlto || 'Social 2';

  if(result.s1.dentro){
    result.categoriaFinal = 'bt-social1';
    return result;
  }

  // --- Testar Social 2 ---
  const r2 = REGRAS_SOCIAL['bt-social2'];
  const s2DentroSmax = smax >= r2.smaxMin && smax <= r2.smaxMax;
  const s2DentroConsumo = consumoMensal <= r2.consumoMax;
  result.s2.dentro = s2DentroSmax && s2DentroConsumo;
  result.s2.falhaSmax = !s2DentroSmax;
  result.s2.falhaConsumo = !s2DentroConsumo;
  result.s2.falhaAmbos = !s2DentroSmax && !s2DentroConsumo;
  result.s2.sugestao = r2.sugestaoSmaxAlto || 'BT Doméstico Monofásico';

  if(result.s2.dentro){
    result.categoriaFinal = 'bt-social2';
    return result;
  }
...
function onFieldChange(e){
  ...
  dev[field] = val;
  save();
  // Não chama renderAll()/renderInventory() aqui — isso recriaria todos os
  // inputs do zero (innerHTML = '') e faria o utilizador perder o foco a
  // cada tecla digitada. Em vez disso, actualiza só os textos derivados
  // desta linha (W total) e os totais/cards globais que dependem disto,
  // mantendo os inputs intactos no DOM.
  const row = e.target.closest('.device-row');
  if(row){
    const wTotal = dev.potencia * dev.quantidade;
    const subEl = row.querySelector('.d-sub');
    if(subEl) subEl.innerHTML = `${dev.quantidade} × ${fmt(dev.potencia,0)} W = <strong class="mono">${fmt(wTotal,0)} W</strong>`;
    row.setAttribute('data-tooltip',
      `${dev.nome} · ${(CATEGORIAS[dev.categoria]||CATEGORIAS.personalizado).label} — Potência unitária ${fmt(dev.potencia,0)} W × ${dev.quantidade} un. = ${fmt(wTotal,0)} W instalados. Ajuste a potência ou a quantidade se necessário.`);
  }
  renderSmaxCard();
  renderResults();
}
...
function renderHours(){
  ...
  listHours.querySelectorAll('input[data-field="horas"]').forEach(inp=>{
    inp.addEventListener('input', (e)=>{
      ...
      dev.horas = val;
      save();
      // Não chama renderAll()/renderHours() aqui — recriaria o input e os
      // chips do zero, fazendo perder o foco a cada tecla. Actualiza só o
      // kWh/d desta linha, o estado activo dos chips, e os totais globais.
      const row = e.target.closest('.device-row');
      if(row){
        const readout = row.querySelector('.kwh-readout');
        if(readout) readout.textContent = fmt(dailyKwh(dev)) + ' kWh/d';
        row.querySelectorAll('.chip').forEach(chip=>{
          const ativo = parseFloat(chip.getAttribute('data-hours')) === val;
          chip.classList.toggle('active', ativo);
          chip.setAttribute('aria-pressed', ativo ? 'true' : 'false');
        });
        row.setAttribute('data-tooltip',
          `${dev.nome} — ${fmt(dev.potencia*dev.quantidade,0)} W × ${fmt(dev.horas,1)} h/dia = ${fmt(dailyKwh(dev))} kWh/dia. Ajuste as horas para ver o impacto no custo.`);
      }
      renderResults();
    });
...
function renderCardPotencia(){
  ...
  // Usa a categoria FINAL (decidida por Smax+Consumo) em vez da escolhida
  const catFinal = hasCategoria ? determinarCategoriaFinal().categoriaFinal : null;
  const usaSmax = hasCategoria && CATEGORIAS_VALIDADAS_POR_SMAX.includes(catFinal);
...
function renderSubCategorias(grupoKey){
  ...
  grupo.categorias.forEach((key)=>{
    ...
    const taxaTooltip = t.taxaFixa > 0 ? `Taxa fixa: ${fmt(t.taxaFixa,2)} Kz/kVA · ` : '';
    const tooltip = `${t.label} — ${taxaTooltip}Tarifa: ${fmt(t.tarifaKwh,2)} Kz/kWh. ${t.descricao}`;
