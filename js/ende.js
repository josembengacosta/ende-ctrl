 /**/
        ...
        (function () {
            ...
            function irParaBoasVindas() {
                // pára qualquer fala e temporizador pendente de outros ecrãs antes de trocar
            ...
            /* ============================================================
               TELA 1 — texto do robô, leitura por voz (Web Speech API)
               ============================================================ */
            ...
            document.getElementById('btn-falar').addEventListener('click', function () {
                const btn = this;
                if (!('speechSynthesis' in window)) {
                    Swal.fire({ icon: 'info', title: 'Leitura por voz indisponível', text: 'Este navegador não suporta a Web Speech API.' });
                    return;
                }
                if (window.speechSynthesis.speaking) {
                    window.speechSynthesis.cancel();
                    btn.classList.remove('a-falar');
                    btn.setAttribute('aria-pressed', 'false');
                    return;
                }
                const utter = new SpeechSynthesisUtterance(TEXTO_BOAS_VINDAS);
                utter.lang = 'pt-PT';
                if (vozPortuguesa) utter.voice = vozPortuguesa;
                utter.rate = 0.98;
                utter.pitch = 1.02;
                utter.onstart = () => { btn.classList.add('a-falar'); btn.setAttribute('aria-pressed', 'true'); };
                utter.onend = () => { btn.classList.remove('a-falar'); btn.setAttribute('aria-pressed', 'false'); };
                utter.onerror = () => { btn.classList.remove('a-falar'); btn.setAttribute('aria-pressed', 'false'); };
            ...
            let state = { devices: [], tariff: 14.30, advice: '' };
            let currentTab = 0;
            ...
            /* ============================================================
               FÓRMULA OFICIAL DE CÁLCULO (conforme solicitado):
               Consumo diário (kWh) = Potência (W) × Quantidade × Horas/dia ÷ 1000
               Custo diário (Kz)    = Consumo diário (kWh) × Tarifa (Kz/kWh)
               Consumo mensal        = Consumo diário × 30
               Custo mensal           = Consumo mensal × Tarifa
               ============================================================ */
            function dailyKwh(d) {
      ...
      <div class="bk-track"><div class="bk-fill" style="width:${pct}%"></div></div></div>`;
        ...
        <i class="fa-solid fa-circle-plus ci-add"></i></button>`;
            ...
            $('btn-anomaly').addEventListener('click', () => {
                ...
                const extraKzMes = (anomKwhMes - normalKwhMes) * state.tariff;
            ...
            function perguntarSeTirouFoto() {
                Swal.fire({
                    ...
                    if (res.isConfirmed) {
                        // SIM — o robô muda de pose para saudar e agradece a visita
                        const img = $('robo-img-selfie');
                        img.classList.remove('hidden');
                        img.src = 'img/robo-sauda.png';
                        ...
                        // após 5 segundos, o totem limpa os dados e volta ao início para o próximo visitante
                        setTimeout(() => {
                            limparSimuladorParaProximoVisitante();
                            irParaBoasVindas();
                        }, 5000);
